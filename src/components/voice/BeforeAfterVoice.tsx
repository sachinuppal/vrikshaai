...
export const BeforeAfterVoice = () => {
  ...
  return (
    <div ref={containerRef} className="relative min-h-[800px] flex items-center justify-center py-20">
      {/* Chaos State - Before */}
      <motion.div
      ...
      >
        <div className="relative w-full max-w-2xl h-[400px]">
          {/* Chaos label */}
          <motion.div
            className="absolute -top-16 left-1/2 -translate-x-1/2 px-5 py-2.5 rounded-full bg-destructive/10 border border-destructive/20 text-destructive text-base font-medium"
          ...
          {/* Scattered tool cards - larger size */}
          {chaosTools.map((tool, index) => {
            const Icon = tool.icon;
            return (
              <motion.div
                key={index}
                className="absolute left-1/2 top-1/2 w-36 p-4 rounded-xl bg-card border border-destructive/30 shadow-lg"
                style={{
                  x: tool.x,
                  y: tool.y,
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            {chaosTools.map((_, i) =>
              chaosTools.slice(i + 1).map((_, j) => (
                <motion.line
                  key={`${i}-${j}`}
                  x1={`${50 + chaosTools[i].x / 5}%`}
                  y1={`${50 + chaosTools[i].y / 4}%`}
                  x2={`${50 + chaosTools[i + j + 1].x / 5}%`}
                  y2={`${50 + chaosTools[i + j + 1].y / 4}%`}
      ...
      >
        <div className="text-center">
          {/* Success label */}
          <motion.div
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-base font-medium mb-10"
          >
          ...
          {/* Unified card - larger */}
          <motion.div
            className="relative p-10 rounded-3xl bg-gradient-to-br from-primary/10 via-card to-orange-500/10 border border-primary/30 shadow-2xl max-w-lg mx-auto"
            initial={{ y: 20 }}
            animate={{ y: 0 }}
            transition={{ type: 'spring', stiffness: 100 }}
          >
            {/* Enhanced glow effect */}
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-primary/30 to-orange-500/30 blur-2xl -z-10" />
            
            <motion.div
              className="w-24 h-24 mx-auto rounded-2xl bg-gradient-to-br from-primary to-orange-500 flex items-center justify-center mb-8"
              animate={{
                boxShadow: [
            ...
            <h3 className="text-3xl font-bold mb-3">One Platform</h3>
            <p className="text-muted-foreground text-lg mb-8">All channels. Zero chaos.</p>
